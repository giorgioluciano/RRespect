...
getContSpec <- function(par) {
 ...
 kernMat <- getKernMat(s, t)  # Assuming getKernMat is defined elsewhere
 print("kernMat")
 print(kernMat)
 kernMat=t(kernMat)
 print(class(kernMat))
 ...
 if (par$verbose) {
   ...
   write.table(data.frame(t=t, K=K),
 ...
 if (par$plotting) {
   ...
   p2 <- ggplot(data.frame(t=t)) +
     geom_point(aes(x=t, y=Gexp)) +
     geom_line(aes(x=t, y=K), color="black") +
     scale_x_log10() +
     scale_y_log10() +
     labs(x="t", y="G(t)") +
     theme_minimal()
