...
function [Nopt g tau error] =  discSpec(par)
	...
	[w, Gp, Gpp, s, H] = ReadData(par.GstFile,'output/H.dat');
	...
	Nmax  = min(floor(3*log(max(w)/min(w))),n/4); % maximum N
	...
	for i = 1:npts
		...
		[g tau ev(i) condN(i)] = MaxwellModes(z, w, Gp, Gpp,par.prune);   % Get g_i
	end
	...
	[g tau error cKp] = MaxwellModes(z, w, Gp, Gpp,par.prune);   % Get g_i
...
%~
		...
		%~ set(H2,'LineStyle','--');
		%~ hold on
		%~ plot(Nopt,ev(find(Nv==Nopt)),'r*')
		%~ hold off

		subplot(2,1,2)

		PlotMaxwellModes(g, tau, w, Gp, Gpp)
		%~ hold on
		%~ K = kernel(H,w,s);
...
% Read Data Files
%
function   [w, Gp, Gpp, s, H] = ReadData(fNameGst,fNameH)

	[w Gexp] = GetExpData(fNameGst);
	n        = numel(w);
	Gp       = Gexp(1:n);
	Gpp      = Gexp(n+1:end);
