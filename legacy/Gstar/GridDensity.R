...
GridDensity <- function(x, px, N) {
  ...
  pint <- pint / total
  ci   <- ci   / total
  # ensure ci is strictly increasing (remove flat regions)
  ci[1] <- 0
  ci[npts] <- 1
  # remove duplicate ci values to allow interpolation inversion
  ...
  if (N > 2) {
    ...
    zij[2:N]       <- sapply(beta_ij, fint_inv)
    beta_z         <- seq(1, N - 2) * alfa
    z[2:(N-1)]     <- sapply(beta_z,  fint_inv)
  }
