...
def kernel_prestore(H, kernMat, *argv):
    logging.debug(f"kernel_prestore called with H shape: {H.shape}, kernMat shape: {kernMat.shape}")
    ...
    result = np.dot(kernMat, np.exp(H)) + G0
    logging.debug(f"kernel_prestore output shape: {result.shape}")
    return result
