...
InitializeH <- function(Gexp, wexp, s, kernMat, ...) {
  ...
  tryCatch({
    if (length(args) > 0) {
      ...
      result <- getH(lam, Gexp, wexp, H, kernMat)
      print("H")
      print(H)
      Hlam <- result$Hlam
      message("Debug: getH returned with Hlam length: ", length(Hlam))
...
getBmatrix <- function(H, kernMat, Gexp, wexp, ...) {
  ...
  tryCatch({
    if (length(args) > 0) {
      ...
      r <- wexp * (1 - kernel_prestore(H, kernMat) / Gexp) #*ok checked 1:50
    }
    
    # Compute matrix B
    B <- t(Jr) %*% Jr + diag(as.vector(t(r) %*% Jr)) #ok checked 1:50
...
lcurve <- function(Gexp, wexp, Hgs, kernMat, par, ...) {
  ...
  logDetN <- determinant(Amat)$modulus #difference from python need to change it into numeric
  logDetN <- as.numeric(logDetN)
  ...
  for (i in rev(1:length(lam))) {
    ...
    if (par$plateau && length(args) > 0) {
      H <- getH(lamb, Gexp, wexp, Hgs, kernMat, G0)
    ...
    logDetC <- as.numeric(logDetC) #difference from python need to explicitly transform in numeric
...
getH <- function(lam, Gexp, wexp, H, kernMat, ...) {
  ...
  result <- tryCatch({
    if (length(args) > 0) {
      ...
      return(list(Hlam = res_lm$par[1:(length(Hplus) - 1)], G0 = res_lm$par[length(Hplus)]))
...
residualLM <- function(H, lam, Gexp, wexp, kernMat) {
  ...
  if (length(H) > ns) {
    G0 <- tail(H, 1)  
    H <- head(H, -1)  
    Kh0 <- kernel_prestore(H, kernMat, G0)
    
    r[1:n] <- wexp * (1-(Kh0/Gexp))
    
  } else {
    
    Kh <- kernel_prestore(H, kernMat) 
    r[1:n] <- wexp*(1 - (Kh/Gexp))
...
jacobianLM <- function(H, lam, Gexp, wexp, kernMat) {
 ...
 diag(L[-1, -ncol(L)]) <- 1   # Diagonale sopra la principale
 diag(L[-nrow(L), -1]) <- 1   # Diagonale sotto la principale
 diag(L) <- -2   
 L <- L[2:(nl+1), ]
...
kernelD <- function(H, kernMat) {
 ...
 Hsuper <- matrix(rep(exp(H), each =2*n), nrow = n, byrow = TRUE)
...
getContSpec <- function(par) {
 ...
 #
 # Input: "par" list from "inp.dat" which specifies GexpFile (often 'Gt.dat')
 # Return: H and lambdaC; the latter can be used to microspecify lambdaC as desired
 ...
 data <- GetExpData(par$GexpFile)  # Assuming GetExpData is defined elsewhere
 w <- data$w
 Gexp <- data$Gst
 wexp <- data$wexp 
 ...
 kernMat <- getKernMat(s, w)  # Assuming getKernMat is defined elsewhere
 print("kernMat")
 print(kernMat)
 print(class(kernMat))
 tic <- Sys.time()
 ...
 if (par$plateau) {
   result <- InitializeH(Gexp, wexp, s, kernMat, min(Gexp))
   ########################################################
   Hgs <- result$Hlam
   print(Hgs)
   G0 <- result$G0
   } else {
   result <- InitializeH(Gexp, wexp, s, kernMat)
   #
   Hgs=result$Hlam
   print("Hgs")
   print(Hgs)
 ...
 if (par$lamC == 0) {
   if (par$plateau) {
     print(Hgs)
     result <- lcurve(Gexp, wexp, Hgs, kernMat, par, G0)
   } else {
     result <- lcurve(Gexp, wexp, Hgs, kernMat, par)
   }
   lamC <- result$lamM
   lam <- result$lam
   rho <- result$rho
 ...
 if (par$plateau) {
   logging::logdebug(sprintf("Before getH call (with plateau): lamC=%.3e", lamC))
   result <- getH(lamC, Gexp, wexp, Hgs, kernMat, G0)
   H <- result$H
   G0 <- result$G0
   logging::logdebug(sprintf("After getH call (with plateau): G0=%.3e", G0))
   cat(sprintf("G0 = %.3e ...", G0))
 } else {
   logging::logdebug(sprintf("Before getH call (without plateau): lamC=%.3e", lamC))
   H <- getH(lamC, Gexp, wexp, Hgs, kernMat)
   logging::logdebug(sprintf("After getH call (without plateau)"))
 }
 ...
 if (par$verbose) {
   ...
   if (par$plateau) {
     ...
     write.table(data.frame(s=s, H=H), 
                 file="output/H.dat",
                 row.names=FALSE,
                 col.names=TRUE,
                 quote=FALSE,
                 sep=" ")
     cat(sprintf("G0 = %.3e\n", G0), file="output/H.dat", append=TRUE)
   } else {
   ...
   write.table(data.frame(w=w, K=K),
               file="output/Gfit.dat",
               row.names=FALSE,
               col.names=TRUE,
               quote=FALSE,
               sep=" ")
   ...
   if (par$lamC == 0) {
     ...
     # print logP
     write.table(data.frame(lam=lam, logP=logP),
                 file="output/logPlam.dat",
                 row.names=FALSE,
                 col.names=TRUE,
                 quote=FALSE,
                 sep=" ")
     ...
     # print rho-eta
     write.table(data.frame(lam=lam, rho=rho, eta=eta),
                 file="output/rho-eta.dat",
                 row.names=FALSE,
                 col.names=TRUE,
                 quote=FALSE,
                 sep=" ")
 ...
 if (par$plotting) {
   ...
   p1 <- ggplot(data.frame(s=s, H=H), aes(x=s, y=H)) +
     geom_point() +
     geom_line() +
     scale_x_log10() +
     labs(x="s", y="H(s)") +
   ...
   if (par$lamC == 0) {
     ...
     error_df <- data.frame(
       s = rep(s, 2),
       y = c(Hm + 2.5*dH, Hm - 2.5*dH),
       type = rep(c("upper", "lower"), each=length(s))
     )
     
     p1 <- p1 + geom_line(data=error_df, aes(x=s, y=y, group=type), 
                          color="gray", alpha=0.5)
   }
   
   ggsave("output/H.pdf", p1)
   ...
   data_plot <- data.frame(w = w, Gexp = Gexp, K = K)
   
   p2 <- ggplot(data_plot) +
     geom_point(aes(x=w, y=Gexp)) +
     geom_line(aes(x=w, y=K), color="black") +
     scale_x_log10() +
     scale_y_log10() +
     labs(x="t", y="G(t)") +
     theme_minimal()
   
   ggsave("output/Gfit.pdf", p2)
   
   # if lam not explicitly specified then print rho-eta.pdf
   if (exists("lam")) {
     p3 <- ggplot(data.frame(rho=rho, eta=eta)) +
       geom_point(aes(x=rho, y=eta), shape="x") +
       geom_line(aes(x=rho, y=eta)) +
       scale_x_log10() +
       scale_y_log10() +
       labs(x="ρ", y="η") +
       theme_minimal()
     
     rhost <- exp(approx(log(lam), log(rho), xout=log(lamC))$y)
     etast <- exp(approx(log(lam), log(eta), xout=log(lamC))$y)
     
     p3 <- p3 + geom_point(data=data.frame(x=rhost, y=etast),
                           aes(x=x, y=y),
                           color="black",
                           size=3)
     
     ggsave("output/rho-eta.pdf", p3)
   } else {
     cat("lamC prespecified, so not printing rho-eta.pdf/dat\n")
   }
 }
 ...
 logging::loginfo("Finished getContSpec")
