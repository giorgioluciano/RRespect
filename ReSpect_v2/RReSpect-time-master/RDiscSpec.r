...
FineTuneSolution <- function(tau, t, Gexp, wexp, isPlateau, estimateError = FALSE) {
  ...
  tryCatch({
    ...
    if (estimateError) {
      ...
      residual_function <- function(par) {
        res_tG(par, texp = t, Gexp = Gexp, wexp = wexp, isPlateau = isPlateau)
      }
      
      
      J <- jacobian(func = residual_function, x = coef(res))  
    ...
    # Gestione degli errori (ritorna la soluzione originale in caso di fallimento)
  })
