# 7/2023: adding weights, and better encapsulation
...
def initializeDiscSpec(par):
    ...
    """Returns:
        (*)	the experimental data: w, Gexp, wexp
    ...
    return w, Gexp, wexp, s, H, Nv, Gc, Cerror

def MaxwellModes(z, w, Gexp, wexp, isPlateau):
    """
    ...
    #
    g, error, condKp = nnLLS(w, tau, Gexp, wexp, isPlateau)

    # first remove runaway modes outside window with potentially large weight
    izero = np.where(np.logical_or(max(w)*np.array(tau) < 0.02, min(w)*np.array(tau) > 50.))
...
def nnLLS(w, tau, Gexp, wexp, isPlateau):
    """
    #
...
def FineTuneSolution(tau, w, Gexp, wexp, isPlateau):
    ...
    # begin encapsulation
    def res_wG(tau, wexp, Gexp, wt_exp, isPlateau):
        """
            here wexp = omega_experimental; so using wt_exp for weghts
        ...
        residual = wt_exp * (Gmodel/Gexp - 1.)
            
        return residual    	   
    ### end encapsulation

    success   = False
    initError = np.linalg.norm(res_wG(tau, w, Gexp, wexp, isPlateau))
    ...
    g, tau, _, _ = MaxwellModes(np.log(tau), w, Gexp, wexp, isPlateau)   # Get g_i, taui
    finalError   = np.linalg.norm(res_wG(tau, w, Gexp, wexp, isPlateau))
...
def getDiscSpecMagic(par):
    ...
    w, Gexp, wexp, s, H, Nv, Gc, Cerror = initializeDiscSpec(par)
    ...
    for ib, wb in enumerate(wtBase):
        ...
        for i, N in enumerate(Nv):
            ...
            g, tau, ev[i], _ = MaxwellModes(z, w, Gexp, wexp, par['plateau'])    # get g_i
            nzNv[i]                 = len(g)
    ...
    g, tau, error, cKp   = MaxwellModes(z, w, Gexp, wexp, par['plateau'])     # Get g_i, taui	
    succ, gf, tauf       = FineTuneSolution(tau, w, Gexp, wexp, par['plateau'])
    if succ:
    ...
    while min(tauSpacing) < par['minTauSpacing'] and itry < 3:
        ...
        succ, g, tau  = FineTuneSolution(tau, w, Gexp, wexp, par['plateau'])		
        if succ:
            g   = gf.copy(); tau = tauf.copy()
